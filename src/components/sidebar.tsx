  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 p-2 space-y-0.5 mt-1">
        ...
        {[
          { label: 'Tasks', href: '/dashboard', icon: <CheckSquare size={18} /> },
          { label: 'Exams', href: '/dashboard', icon: <BookOpen size={18} /> },
          { label: 'Reminders', href: '/dashboard', icon: <Bell size={18} />, badge: 2 },
        ].map((item) => (
          <Link
            key={`tool-${item.label}`}
            href={item.href}
        ...
        <div className={`flex items-center gap-2.5 px-3 py-2 rounded-lg ${collapsed ? 'justify-center px-0' : ''}`}>
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-zinc-200 truncate">Alex Rivera</p>
              <p className="text-xs text-zinc-500 truncate">alex@dev.io</p>
            </div>
          )}
          {!collapsed && (
